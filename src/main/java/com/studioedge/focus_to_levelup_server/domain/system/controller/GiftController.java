...
import com.studioedge.focus_to_levelup_server.domain.system.dto.request.GiftSubscriptionRequest;
...
public class GiftController {
    ...
    @Operation(
            summary = "구독권 선물",
            description = "특정 유저에게 구독권을 선물합니다. 우편함으로 전달됩니다."
    )
    @PostMapping("/subscription")
    public ResponseEntity<CommonResponse<GiftResponse>> giftSubscription(
            @AuthenticationPrincipal Member sender,
            @Valid @RequestBody GiftSubscriptionRequest request
    ) {
        GiftResponse response = giftService.giftSubscription(
                sender.getId(),
