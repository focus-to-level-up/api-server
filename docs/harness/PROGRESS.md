...
- API/Admin/Batch 멀티모듈 구조에 맞춰 각 모듈을 독립 배포할 수 있는 CI/CD 파이프라인을 구축한다.
  - API는 기존 blue-green 배포 흐름을 유지한다.
  - Admin과 Batch는 API와 독립적으로 배포 가능한 흐름을 만든다.
  - 세 모듈의 테스트, bootJar, 이미지 빌드, 배포 대상, 환경변수 주입 방식을 분리한다.
- 친구초대 기능을 프론트와 연동하여 적용한다.
  - 기존 API 상태를 점검하고 필요한 서버 응답/검증/보상 흐름을 정리한다.
  - 프론트 연동에 필요한 요청/응답 형식과 예외 케이스를 확인한다.
- 출석 기능을 프론트와 연동하여 적용한다.
  - 오늘 날짜 기준 출석 여부, 보상 지급 여부, 중복 출석 방지 정책을 점검한다.
  - 프론트 연동에 필요한 조회/처리 API와 응답 형식을 확인한다.
- 위 목표 완료 후 챗봇 기능, 캐릭터 추가, Batch 시스템 안정화를 병렬 후보 작업으로 검토한다.
  - 챗봇/RAG는 별도 서비스 또는 별도 레포지토리 후보로 유지한다.
  - 캐릭터 추가는 디자인/리소스/서버 데이터 구조를 함께 검토한다.
  - Batch 안정화는 멱등성, 리그/시즌 전환 안정성, 장애 후 재실행 가능성을 필수 개선 과제로 다룬다.
...
- Batch Step 안정화는 현재 최우선 작업인 멀티모듈 CI/CD, 친구초대, 출석 기능 연동 이후 필수 작업으로 진행한다.
- 보상, 통계, 우편 생성은 재실행되어도 중복 생성되지 않도록 멱등성 키와 DB 제약을 설계한다.
  - 주간 보상: 회원과 기준 주차 단위
  - 길드 주간 보상: 길드와 기준 주차 단위
  - 시즌 보상: 회원, 시즌, 보상 종류 단위
  - 월간 통계: 회원과 기준 월 단위
- 리그/시즌 전환은 삭제 후 재생성이 아니라 준비, 검증, 활성화 상태 전환 구조로 개선한다.
  - 기존 활성 리그/시즌은 새 데이터가 완성되기 전까지 유지한다.
  - 새 리그/시즌은 준비 상태로 생성하고 검증 완료 후 활성화한다.
  - 중간 장애 후 재실행 시 이미 처리된 데이터는 건너뛰고 미완료 데이터만 보완하도록 설계한다.
- Batch 장애 대응 품질을 높인다.
  - Step별 성공/실패/skip 건수를 기록한다.
  - 보상/통계/리그 전환 실패 시 재실행 시나리오를 문서화한다.
  - FCM 알림 중복 발송 방지를 위한 발송 이력 또는 기준 키를 검토한다.
...
- 현재 `levelup-infra:client` 내부의 Firebase/FCM/S3 클라이언트 구성은 Batch 독립 실행을 우선하기 위해 유지한다.
- 향후 API/Admin/Batch 독립 배포 플로우가 안정화된 뒤 `levelup-infra:client` 하위 클라이언트를 제공자 또는 책임 단위로 다시 분리한다.
  - 예: Firebase FCM 전송, 사전예약 Firebase 조회, S3 업로드 클라이언트
  - 분리 시 각 애플리케이션 모듈이 실제 필요한 외부 클라이언트만 의존하도록 조정한다.
